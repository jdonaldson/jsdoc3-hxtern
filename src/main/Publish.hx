...
class Publish {
        ...
        taffy_items.each(function(x,y){
            ...
            switch(x.docletType()){
                ...
                case DocletClass(doc) : {
                    ...
                    for (t in p.tags){
                        switch(t.title){
                            case 'extends'                  : extend = renderType(t.type);
                            case 'implements'               : {
                                implement = renderType(t.type);
                            }
                    ...
                    var clazz = makeClazz(name, doc, implement, extend);
                    var param_list = params.join(', ');
    ...
    public static function render(pack : Pack, cwd : String){
        ...
        for (c in pack.classes.keys()){
            ...
            var extends_phrase = '';
            if (clazz.implement != null){
                extends_phrase = 'extends ${clazz.implement} ';
            }
     ...
     **/
    public static function makeClazz(memberof : String, doclet : Doclet, ?is_private : Bool, ?implement : String, ?extend : String ): Clazz {
        if (memberof == null) {
            trace(doclet);
        ...
        if (implement!= null) clazz.implement = implement;
        if (extend != null) clazz.extend = extend;
    ...
    static function lc(arg: String) return ~/^[a-z]/.match(arg);
    static function uc(arg: String) return ~/^[A-Z]/.match(arg);
    static function fixDescription(description:String) : String {
        var fixed_description = description.split('\n').join('\n\t  ');
        return '\t/**\n\t  $fixed_description\n\t */\n';
    }
    static function titleCase(arg: String) : String {
