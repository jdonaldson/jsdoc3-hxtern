...
Publish.publish = function(taffy,opts,tutorial) {
	...
	taffy_items.each(function(x,y) {
		...
		case 3:
			...
			if(is_constructor) {
				console.log(Publish.fixDoctrine(x.comment));
				var cls_pack = Publish.classModule(doc.memberof,null,doc.name);
...
Publish.renderType = function(type) {
	...
	case 13:
		...
		if(count == 0) throw "No types in uniontype for $type"; else if(count == 1) return Publish.renderType(type1.elements[0]); else if(count > 8) throw "Too many types in union type $type"; else return "Hxtern.Any" + count + "<" + types + ">";
		break;
	case 7:
		var type1 = $e[2];
		return "Dynamic";
